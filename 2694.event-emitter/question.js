...
class EventEmitter {
  ...
  subscribe(eventName, callback) {
    const listenersForThisEvent = this.listeners.get(eventName);
    let callbackId = 1;
    if (!listenersForThisEvent) {
      this.listeners.set(callbackId, callback);
    } else {
      callbackId = listenersForThisEvent.size + 1;
      listenersForThisEvent.set(callbackId, callback);
    }
    return {
      unsubscribe: () => {
        this.listeners.get(eventName).delete(callbackId);
      },
    };
  }
...
showLogs(
  ...
  {
    data: [undefined],
    structure: ["string", "Function", "string", "Array"],
  },
  {
    data: [
      [
...
console.log("点击跳转到题目提交: https://leetcode.cn/problems/event-emitter/");
